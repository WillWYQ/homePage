...
**管辖边界:这条规格管的是"读数",不是"一切 mono 文字"。** 读数与内容同处一行时,**只有读数那部分受本规格管辖**,内容部分按内容对待:

| 角色 | 规格 | 例 |
|---|---|---|
| 读数(仪器在报数) | `text-xs` / `white/40` | 编号 `REM-007`、日期、`SEV-3`、`last updated`、EXIF |
| 内容(有人在说话) | `white/70`,行内可点 | /notes 编目行的标题 |
| 交互性 mono(门牌) | `white/60`(对比度下限,§HOME 4.2) | 走廊导航五扇门 |
| 屏身份 | `white/70` | 实验详情页 HUD 的 `EXP-001` 徽标——它是这一屏的名字,不是读数 |

三档灰阶一律不设字号阶。**一行里出现两种灰是对的**——层级等于信息的真实层级:仪器在报数,内容在说话。
...
> 完整规格:**[SOUND-DESIGN.md](SOUND-DESIGN.md)**(声学身份、实验规格、音频引擎),冲突时以它为准。

三条法则:**① chrome 永远静音**(界面层任何元素不发声,引入组件的音效一律关);**② 声音是仪器不是装饰**(唯一发声位置是 /lab 实验内部,且必须是真实信号);**③ 永远 opt-in、状态永远可见**(HUD 常驻 `sound: on` 读数,`m` 即静,默认永远是关)。声学身份:全合成零采样、低频域(基频谐波系 A=110Hz)、缓起缓落(attack ≥50ms)——"夜里的速度"的听觉版。
...
/reel          卷带间   精选与日志:反复回去听的 / 这段时间在听什么
...
> 本房间有独立规格:**[HOME-DESIGN.md](HOME-DESIGN.md)**(门厅/枢纽双身份 + 开场状态机 + 分时段仪表读数),冲突时以它为准。以下为概要。

- 单屏不滚动:标题区(现状不动)+ mono 导航五扇门 + 左上角一条仪表读数(本地时间分时段文案 + 最新记录,按"nights ago"计时)+ 全站统一页脚。
- Vortex 开场只演给"第一次进楼":sessionStorage 一晚一次,reduced-motion / 无 JS / yueqiao 构建均不播;播放 1.8s 后出现 `tap to enter ↵` 跳过提示。
- 之后的每次路过,走廊是零成本枢纽:无入场动画,回程成本 ≈ 一次点击 + 300ms 交叉淡入。
- 2026-07-05 修订:走廊的"酷"只花在状态上,不花在动画上——灯随访客时段调光(白天真的暗)、门缝读数(hover 门牌时读数行换成该房间的构建期真实库存)、04:04 一分钟 reality-check 彩蛋。三者全住在既有元素内部,DOM 与组件零新增;否决的炫技提案记录在 HOME-DESIGN §9。
- 未上线房间的门牌不渲染(红线 3);走廊永远不再接受新的视觉组件。
...
- **列表页:** 按年分组,每行一条 mono 编目行,类型只靠编号前缀区分,不做彩色徽标(单一强调色,§2):
  ...
  ```
  **一行两档灰**(§2 管辖边界):前缀的编号/日期/SEV 是读数,`white/40`;**标题是内容,`white/70`,且标题即链接**(整行可点,但视觉重心在标题)。档案室是目录不是脚注——整行压成 `white/40` 会让主体内容看起来像出错,也看不出可点。hover 时标题变白,读数不变(仪器不响应鼠标)。
...
| /reel 卷带间 | **无** | — | — | 精选与日志陈列,不做入场演出;克制感与 /now、/notes 同源 |
