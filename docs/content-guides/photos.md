...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `title` | string | 否 | roll 标题。不写则用目录名。**注意**:`photoSetSchema` 里 `title` 是 `.trim().min(1).optional()`——不写这个 key 完全没事,但写成空字符串 `title: ""` 会校验失败、**build 报错**,跟这张表里其他"不填就静默回退"的可选字段不一样。 |
...
4. 脚本做的事:检查 iCloud 文件是否已下载到本地(dataless 会等待物化,超时
   ...
   sharp 生成 480/960/1600/2400 四档 webp(`scripts/sync-images/process-image.mjs`
   的 `resizeTiers()`:比源图更宽的档位直接跳过、不放大;源图本身比最小档
   480px 还窄的,`scripts/sync-images.mjs` 会整张图跳过并打印警告,不生成任何
   档位——仓库里 `content/image-manifest.json` 现有的那张 960px 宽的真实照片,
   `sizes` 就只有 `480`/`960` 两档,没有 `1600`/`2400`)→ 算 blurhash 占位图 →
   提取 EXIF → 按内容哈希幂等上传到 R2(已存在的 key 直接跳过,不重复上传)→
   把结果写进 `content/image-manifest.json`。
5. 提交 `content/image-manifest.json` 的改动(它是仓库跟踪的产物,图片二进制本身
   不进 git)。

## 坑
- **最容易踩、也最不容易发现的坑**:`buildFrame()`(`lib/content.ts` 约
  485-528 行)对每张图片先查 `content/image-manifest.json` 里有没有
  `photos/<roll>/<file>` 这个 key。查不到时,**只在 `NODE_ENV === "production"`
  这个分支下**直接返回 `null`——这一帧被静默丢弃,不渲染、不报错、不警告。
  `next dev` 下不会走到这个分支:代码会给 `content/photos/` 建一个
  `/_dev-photos` 符号链接,manifest 里没有条目也照样直接读本地原图渲染出来
  (只是没有尺寸/EXIF/blurhash,§6.4/DESIGN §4.6 的 dev 回落)。实际后果:
  把新照片扔进 `content/photos/<roll>/` 之后,忘了跑 `npm run sync:images`,
  或者跑了但没提交重新生成的 `content/image-manifest.json`——`next dev` 下
  页面看起来完全正常,一旦部署到生产,这些照片会**悄无声息地从页面上消失**,
  没有任何报错或警告能提示你漏了这一步。
