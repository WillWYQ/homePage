...
- [ ] **Step 1: Confirm the current lockfile drift risk exists**
...
- [ ] **Step 4: Verify pnpm still installs cleanly without the npm lockfile**
...
- [ ] **Step 5: Verify the new script runs**
...
- [ ] **Step 1: Confirm current workflow still references npm**
...
- [ ] **Step 2: Replace the install/build steps with pnpm equivalents**
...
- [ ] **Step 4: Simulate the install+build steps locally**
...
- [ ] **Step 1: Confirm no CI currently runs on pull requests**
...
- [ ] **Step 4: Run every step of the new workflow locally, in order**
...
- [ ] **Step 1: Confirm the working tree matches what was committed**

Run: `git status`
Expected: clean (nothing to commit) — Tasks 1–3 each committed their own changes.

- [ ] **Step 2: Confirm only one lockfile remains**

Run: `git ls-files | grep -i lock`
Expected: only `pnpm-lock.yaml` (no `package-lock.json`).

- [ ] **Step 3: Confirm both workflow files are valid and reference pnpm, not npm**
...
- [ ] **Step 4: Re-run the full local sequence one more time from a clean state**
